  ...
  module Output
    ...
    class Htmlout < Baseout
      ...
      def initialize(events, hosts, options)
        ...
        FileUtils.cp(File.expand_path("#{$root_dir}/lib/js/jquery.dataTables.js"),
                     "#{options[:output]}/jquery.dataTables.js")
        ...
        FileUtils.cp(File.expand_path("#{$root_dir}/lib/images/back_disabled.png"),
                     "#{options[:output]}/back_disabled.png")
        FileUtils.cp(File.expand_path("#{$root_dir}/lib/images/back_enabled.png"),
                     "#{options[:output]}/back_enabled.png")
        FileUtils.cp(File.expand_path("#{$root_dir}/lib/images/back_enabled_hover.png"),
                     "#{options[:output]}/back_enabled_hover.png")
        FileUtils.cp(File.expand_path("#{$root_dir}/lib/images/forward_disabled.png"),
                     "#{options[:output]}/forward_disabled.png")
        FileUtils.cp(File.expand_path("#{$root_dir}/lib/images/forward_enabled.png"),
                     "#{options[:output]}/forward_enabled.png")
        FileUtils.cp(File.expand_path("#{$root_dir}/lib/images/forward_enabled_hover.png"),
                     "#{options[:output]}/forward_enabled_hover.png")
        FileUtils.cp(File.expand_path("#{$root_dir}/lib/images/sort_asc.png"), "#{options[:output]}/sort_asc.png")
        FileUtils.cp(File.expand_path("#{$root_dir}/lib/images/sort_asc_disabled.png"),
                     "#{options[:output]}/sort_asc_disabled.png")
        ...
        FileUtils.cp(File.expand_path("#{$root_dir}/lib/images/sort_desc_disabled.png"),
                     "#{options[:output]}/sort_desc_disabled.png")
      end
      ...
      #
      def run
        print_hosts # generate all the host_*.html files
        print_index # generate the index.html file
        print_vulns # generate all the vuln_*.html files
        print_vuln_overview # generate the vuln_overview.html file
      end
      ...
      def print_index
        File.open("#{@options[:output]}/index.html", 'w') do |f|
          ...
          bar_js(f, 'bar_graph', 'Top 20 Hosts', @hosts.sort_by { |_k, v| v[:total_excl_info] }.reverse.take(20))
          ...
          @events.each do |_k, v|
            ...
            info_total += 1 if (v[:severity]).zero?
          end

          pie_data = []
          pie_data << ['Info', info_total.to_i, 'blue'] if (@options[:severity] <= 0) && (info_total.to_i >= 0)
          pie_data << ['Low', low_total.to_i, 'green'] if (@options[:severity] <= 1) && low_total.to_i.positive?
          pie_data << ['Medium', med_total.to_i, 'orange'] if (@options[:severity] <= 2) && med_total.to_i.positive?
          pie_data << ['High', high_total.to_i, 'red'] if (@options[:severity] <= 3) && high_total.to_i.positive?
          pie_data << ['Critical', crit_total.to_i, 'purple'] if (@options[:severity] <= 4) && crit_total.to_i.positive?

          pie_js(f, 'pie_graph', 'Unique Vulnerability Breakdown', 'Unique Vuln Breakdown', pie_data,
                 "document.location.href = 'vuln_overview.html';")
          ...
          pie_js(f, 'pie_graph2', 'Total Vulnerability Breakdown', 'Total Vuln Breakdown', pie_data,
                 "document.location = href= 'vuln_overview.html';")

          target_lookup = 'var target_lookup = {'
          @hosts.each_with_index do |host, index|
            target_lookup += if host[1][:hostname] == ''
                               "'#{host[1][:ip]}'"
                             else
                               "'#{host[1][:hostname]} (#{host[1][:ip]})'"
                             end
            target_lookup += ": #{host[0]}"
          ...
          ips.sort_by { |ip| ip.split('.').map(&:to_i) }.each do |ip|
            @hosts.select { |_k, v| v[:ip] == ip }.each do |k, v|
              tmp_actual_v_count = 0
              tmp_actual_v_count += v[:info].to_i if (@options[:severity] <= 0) && (v[:info].to_i >= 0)
              tmp_actual_v_count += v[:low].to_i if (@options[:severity] <= 1) && v[:low].to_i.positive?
              tmp_actual_v_count += v[:med].to_i if (@options[:severity] <= 2) && v[:med].to_i.positive?
              tmp_actual_v_count += v[:high].to_i if (@options[:severity] <= 3) && v[:high].to_i.positive?
              tmp_actual_v_count += v[:crit].to_i if (@options[:severity] <= 4) && v[:crit].to_i.positive?
              body += '<tr><td>'
              body += if tmp_actual_v_count.positive?
                        "<a href=\"host_#{k}.html\">#{ip}</a>"
                      else
                        ip
                      end
              body += "</td><td>#{v[:hostname]}</td><td>#{v[:os]}</td><td>#{v[:total_excl_info]}</td></tr>"
            end
          end
          body += '</tbody></table>'
      ...
      def print_vuln_overview
        File.open("#{@options[:output]}/vuln_overview.html", 'w') do |f|
          ...
          @events.each do |k, v|
            ...
            body += "<tr><td><a href=\"vuln_#{k}.html\">#{k}"
            body += "</a></td><td>#{v[:severity]}<td>#{v[:plugin_name]}</td>"
            body += "<td>#{v[:family]}</td><td>"
            impacted_hosts = []
            ...
            impacted_hosts.uniq!
            body += "</td><td>#{impacted_hosts.count}</td></tr>"
          end
          body += '</tbody></table>'
      ...
      def print_vulns
        @events.each do |id, values|
          ...
          File.open("#{@options[:output]}/vuln_#{id}.html", 'w') do |f|
            html_header(f, id.to_s)

            close_html_header(f)

            body = "<a href=\"index.html\">Home</a><br /><div id=\"vuln\" style=\"font-family: Arial, Helvetica, sans-serif\"><div id=\"overview\">Nessus ID: #{id}<br />Name: #{values[:plugin_name]}<br />Severity: #{values[:severity]}<br />Family: #{values[:family]}<br />Ports: "
            impacted_hosts = []
            values[:ports].each_with_index do |(k, v), index|
              body += k.to_s
            ...
            body += "<br /><br />CVE: #{values[:cve]}<br />CVSS Base Score: #{values[:cvss_base_score]}<br />CVSS Vector: #{values[:cvss_vector]}</div>"
            body += '</div>'
            ...
            impacted_hosts.uniq.each do |host|
              values[:ports].each do |k, v|
                v[:hosts].each do |h, w|
                  next unless h == host

                  body += "<tr><td><a href=\"host_#{host}.html\">#{@hosts[host][:ip]}</a></td><td>#{@hosts[host][:hostname]}</td><td>#{@hosts[host][:os]}</td>"
                  body += '<td>' + k.to_s + '</td><td>' + w.to_s.gsub(%r{</?[^>]*>}, '').gsub("\n",
                                                                                              "<br />\n") + "</td></tr>\n"
                end
              end
            end
      ...
      def print_hosts
        @hosts.each do |id, values|
          File.open("#{@options[:output]}/host_#{id}.html", 'w') do |f|
            html_header(f, values[:ip])

            if (values[:total_excl_info]).zero?
              pie_js(f, 'pie_graph', 'Criticality Breakdown', 'Criticality Breakdown',
                     [['Informational ONLY', values[:info].to_i, 'blue']])
            else
              pie_data = []
              if (@options[:severity] <= 0) && (values[:info].to_i >= 0)
                pie_data << ['Info', values[:info].to_i,
                             'blue']
              end
              if (@options[:severity] <= 1) && values[:low].to_i.positive?
                pie_data << ['Low', values[:low].to_i,
                             'green']
              end
              if (@options[:severity] <= 2) && values[:med].to_i.positive?
                pie_data << ['Medium', values[:med].to_i,
                             'orange']
              end
              if (@options[:severity] <= 3) && values[:high].to_i.positive?
                pie_data << ['High', values[:high].to_i,
                             'red']
              end
              if (@options[:severity] <= 4) && values[:crit].to_i.positive?
                pie_data << ['Critical', values[:crit].to_i,
                             'purple']
              end
              pie_js(f, 'pie_graph', 'Criticality Breakdown', 'Criticality Breakdown', pie_data,
                     "document.location.href = '#' + event.point.name;")
            end
            ...
            if (@options[:severity] <= 4) && values[:crit].to_i.positive?
              ...
              @events.sort_by { |_k, v| v[:port].to_s }.each do |vuln_id, vuln_data|
                vuln_data[:ports].each do |k, v|
                  v[:hosts].each do |h, v2|
                    next unless (h == id) && (vuln_data[:severity] == 4)

                    body += '<tr><td><a href="vuln_' + vuln_id.to_s + '.html">' + vuln_id.to_s + '</a></td><td>' + vuln_data[:plugin_name] + '</td><td>' + vuln_data[:synopsis] + '</td><td>' + v2.to_s.gsub(%r{</?[^>]*>}, '').gsub(
                      "\n", '<br />'
                    ) + '</td><td>' + vuln_data[:family] + '</td><td>' + k.to_s + '</td></tr>'
                  end
                end
              end
            ...
            if (@options[:severity] <= 3) && values[:high].to_i.positive?
              ...
              @events.sort_by { |_k, v| v[:port].to_s }.each do |vuln_id, vuln_data|
                vuln_data[:ports].each do |k, v|
                  v[:hosts].each do |h, v2|
                    next unless (h == id) && (vuln_data[:severity] == 3)
            ...
            if (@options[:severity] <= 2) && values[:med].to_i.positive?
              ...
              @events.sort_by { |_k, v| v[:port].to_s }.each do |vuln_id, vuln_data|
                vuln_data[:ports].each do |k, v|
                  v[:hosts].each do |h, v2|
                    next unless (h == id) && (vuln_data[:severity] == 2)
            ...
            if (@options[:severity] <= 1) && values[:low].to_i.positive?
              ...
              @events.sort_by { |_k, v| v[:port].to_s }.each do |vuln_id, vuln_data|
                vuln_data[:ports].each do |k, v|
                  v[:hosts].each do |h, v2|
                    next unless (h == id) && (vuln_data[:severity] == 1)
            ...
            if (@options[:severity] <= 0) && values[:info].to_i.positive?
              ...
              @events.sort_by { |_k, v| v[:port].to_s }.each do |vuln_id, vuln_data|
                vuln_data[:ports].each do |k, v|
                  v[:hosts].each do |h, v2|
                    next unless (h == id) && (vuln_data[:severity]).zero?
      ...
      def close_html_header(fp)
        ...
        return if @options[:indexfile].nil?
      ...
      def bar_js(fp, renderto, title, data)
        ...
        data.each_with_index do |entry, index|
          tmpline = "'"
          tmpline += if entry[1][:hostname] == ''
                       entry[1][:ip]
                     else
                       "#{entry[1][:hostname]} (#{entry[1][:ip]})"
                     end
          tmpline += "'"
      ...
      def pie_js(fp, renderto, title, seriesname, series, clickfunction = nil)
        ...
        fp.puts ",events: { click: function(event) { #{clickfunction}} }" unless clickfunction.nil?
        ...
        series.each_with_index do |val, index|
          tmpline =  "\t\t\t{name: '#{val[0]}', y: #{val[1]}, color: '#{val[2]}'}"
