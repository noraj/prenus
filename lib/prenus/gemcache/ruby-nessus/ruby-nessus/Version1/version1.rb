...
require 'prenus/gemcache/ruby-nessus/ruby-nessus/Version1/host'
require 'prenus/gemcache/ruby-nessus/ruby-nessus/Version1/event'
...
module Nessus
  ...
  module Version1
    ...
    class XML
      ...
      #
      def start_time
        @start_time = DateTime.strptime(@xml.xpath('//NessusClientData//Report//StartTime').inner_text,
                                        fmt = '%a %b %d %H:%M:%S %Y')
      end
      ...
      #
      def stop_time
        @stop_time = DateTime.strptime(@xml.xpath('//NessusClientData//Report//StopTime').inner_text,
                                       fmt = '%a %b %d %H:%M:%S %Y')
      end
      ...
      def event_percentage_for(type, round_percentage = false)
        @sc ||= count_severity
        unless %w[high medium low all].include?(type)
